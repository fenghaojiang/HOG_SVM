...
int main(int argc, char const *argv[])
{
  ...
  //若TRAIN为true，重新训练分类器
	if (TRAIN)
	{
		...
		ifstream finPos(CalculateTrainPosList);//正样本图片的文件名列表
		//ifstream finPos("PersonFromVOC2012List.txt");//正样本图片的文件名列表
		ifstream finNeg(CalculateTrainNegList);//负样本图片的文件名列表
		...
		for (int num = 0; num < PosSamNO && getline(finPos, ImgName); num++)
		{
			...
			ImgName = CalculateTrainPosDir + ImgName;//加上正样本的路径名
		...
		for (int num = 0; num < NegSamNO && getline(finNeg, ImgName); num++)
		{
			...
			ImgName = CalculateTrainNegDir + ImgName;//加上负样本的路径名
			Mat src = imread(ImgName);//读取图片
		...
		if (HardExampleNO > 0)
		{
			...
			for (int num = 0; num < HardExampleNO && getline(finHardExample, ImgName); num++)
			{
				...
				ImgName = "E://HOG_SVM//TestAcc//HardExample//" + ImgName;//加上HardExample负样本的路径名
				Mat src = imread(ImgName);//读取图片
				//resize(src,src,Size(64,128));
